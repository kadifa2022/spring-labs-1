...
public interface MovieRepository extends JpaRepository<Movie, Long> {
    ...
    List<Movie> fetchByPriceBetween(@Param("price1")BigDecimal price1, @Param("price2") BigDecimal price2);
    ...
    // ------------------Native Queries-------------------
    //Write native query that return a movie name
    @Query(value = "SELECT * FROM movie  WHERE name = ?1", nativeQuery = true)
    Optional<Movie> retrieveByName(@Param("name") String name);

    //Write a native query that return the list of movies in a specific range of price
    ...
    List<Movie> retrieveAllByDurationInRange(@Param("durations") List<Integer> durations);
